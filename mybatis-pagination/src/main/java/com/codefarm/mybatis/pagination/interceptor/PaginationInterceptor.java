...
public class PaginationInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (parameter != null) {
			...
			for (Field field : declaredFields) {
				if (field.isAnnotationPresent(OrderBy.class)) {
					if (StringUtils.isNotEmpty((String) Reflections.getFieldValue(parameter, field.getName()))) {
						sb.append(Reflections.getFieldValue(parameter, field.getName()));
						Reflections.setFieldValue(boundSql, "sql", sb.toString());
					}
					break;
				}
			}
		...
		if (parameter != null && parameter instanceof Pagable && ((IPagable) parameter).isPagable()) {
			...
			String pageSql = dialect.getLimitString(sql, (page.getCurrentPage() - 1) * page.getShowCount(),
					page.getShowCount());
	...
	private Dialect getDialect(RoutingStatementHandler statementHandler) {
		...
		try {
			databaseType = Dialect.DialectName
