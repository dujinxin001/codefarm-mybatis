...
public class CriteriaSqlNodeBuilder extends SqlNodeBuilder {
	...
	public SqlNode build(Configuration configuration, MappedMethod method) {
		...
		if (!containCritera(method))
			return new TextSqlNode("");
		contents.add(new TextSqlNode(" WHERE 1=1 AND "));
		// 参数索引，从1开始
		int index = 1;
		String prefix = "";
		for (Parameter parameter : parameters) {
			...
			if (parameter.getType().isAnnotationPresent(Criterias.class)) {
				buildCriteriaFromObject(configuration, contents, prefix, parameter);
			...
			// --------parse Primative paramter
				...
				if (parameter.getType().isArray() || Collection.class.isAssignableFrom(parameter.getType())) {
					...
					contents.add(buildForEachSqlNode(configuration, collection));
				} else {
		...
		return new TrimSqlNode(configuration, new MixedSqlNode(contents), "", null, "", "AND");
	}

	private boolean containCritera(MappedMethod method) {
		Parameter[] parameters = method.getMethod().getParameters();
		for (Parameter parameter : parameters) {
			if (parameter.isAnnotationPresent(Criteria.class)
					|| parameter.getType().isAnnotationPresent(Criterias.class))
				return true;
		}
		return false;
	}

	private void buildCriteriaFromObject(Configuration configuration, List<SqlNode> contents, String prefix,
			Parameter parameter) {
		StringBuilder sb = new StringBuilder();
		...
		for (Field field : fields) {
			...
			String columnName = annotation.column();
			// --------数组参数--------//
			if (field.getType().isArray() || Collection.class.isAssignableFrom(field.getDeclaringClass())) {
				sb.append(columnName);
				sb.append(" in ");
				List<SqlNode> sub = new ArrayList<>();
				sub.add(new TextSqlNode(sb.toString()));
				sub.add(buildForEachSqlNode(configuration, prefix + field.getName()));
				...
				sb.append(annotation.operator().getOperator());
				sb.append(" ");
				if (annotation.operator().equals(Operator.LIKE)) {
					sb.append("CONCAT('%',");
					sb.append("#{");
					sb.append(prefix);
				...
				// sb.append(" AND ");
				String test = buildIfTest(prefix, field);
				contents.add(new IfSqlNode(new TextSqlNode(sb.toString()), test));
			}
